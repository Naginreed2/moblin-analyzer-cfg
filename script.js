...
function renderProfiles(streams) {
  ...
  container.innerHTML = '';
  // sort so enabled=true come first
  streams
    .slice()
...
// Flattened details, with boolean coloring
...
// Clear old inline diffs (not used now)
function clearDiff() {
...
// Helpers for Global Settings sections:
...
// Build a 2-column table from [ [key,value], ... ]
function buildTable(rows) {
  const tbl = document.createElement('table');
  tbl.className = 'global-table';
  const header = `<thead><tr><th>Key</th><th>Value</th></tr></thead>`;
  const body = rows.map(([k,v]) => {
    const isBool = String(v).toLowerCase();
    const cls = (isBool==='true'||isBool==='on')?'value-true'
              : (isBool==='false'||isBool==='off')?'value-false':'';
...
// Nested table helper (for relay etc.)

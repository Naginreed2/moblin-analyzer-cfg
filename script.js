...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // On load, restore config if present
  const stored = localStorage.getItem('moblinConfig');
  if (stored) {
...
function analyzeConfig() {
  ...
  if (!raw) { alert('Please paste or load some JSON first!'); return; }
  let cfg;
  try {
    cfg = JSON.parse(raw);
  } catch (e) {
...
function compareProfiles() {
  ...
  const selected= streams.filter(s => names.includes(s.name));
  localStorage.setItem('profileComparison', JSON.stringify(selected));
...
// Flattened details, no nested tables
...
function renderGlobalSettings(cfg) {
  ...
  for (const [cat, v] of Object.entries(cfg)) {
    ...
    switch (cat) {
      case 'remoteControl':
        renderRemoteControl(container, v);
        break;
      case 'widgets':
        renderWidgets(container, v);
        break;
      case 'bitratePresets':
        renderBitratePresets(container, v);
        break;
      case 'globalButtons':
        renderGlobalButtons(container, v);
        break;
      case 'scenes':
        renderScenes(container, v);
        break;
      case 'gameControllers':
        renderGameControllers(container, v);
        break;
      default:
        if (v && typeof v === 'object') {
          renderGenericObject(container, v);
...
// Nested helpers for each section:
function renderRemoteControl(parent, rc) {
  ...
  if (c.relay) {
    const nested = nestedTable(c.relay);
    rows.push(['client.relay', nested]);
  }
...
function renderBitratePresets(parent, arr) {
  const rows = arr.map(e => ([`${e.bitrate/1e6}Mbps`, e.bitrate]));
  parent.appendChild(buildTable(rows));
}
...
function renderScenes(parent, arr) {
  arr.forEach(s => {
    ...
    Object.entries(s).forEach(([k,v]) => {
      ...
      if (['string','number','boolean'].includes(typeof v)) rows.push([k,v]);
      else rows.push([k, nestedTable(v)]);
    });
    parent.appendChild(buildTable(rows));
  });
}
...
// Build a nested table for object values
function nestedTable(o) {
  let html = `<table class="global-inner-table"><thead>
    <tr><th>Key</th><th>Value</th></tr></thead><tbody>`;
  Object.entries(o).forEach(([k,v]) => {
    if (['string','number','boolean'].includes(typeof v)) {
      const cls = String(v).toLowerCase() === 'true'  ? 'value-true'
                : String(v).toLowerCase() === 'false' ? 'value-false'
                : '';
